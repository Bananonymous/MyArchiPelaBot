...
function buildLink(config, apPort, slotName) {
  const webBase = config.webClientPublicUrl || `http://${config.serverHost}:${config.webClientPort}/`;
  const params = new URLSearchParams({ url: `${config.serverHost}:${apPort}` });
  if (slotName) params.set('slot', slotName);
  return `${webBase}#/?${params.toString()}`;
}

module.exports = { start, buildLink };
