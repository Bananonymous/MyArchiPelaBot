...
// Prevent duplicate listener attachment (e.g. recovery + start logic)
const _attachedToClient = new WeakSet();
...
async function attachGameNotifier(gameId, channel) {
  ...
  if (_attachedToClient.has(apClient)) {
    console.log(`[notifier-${gameId}] Already attached (skipping duplicate handlers)`);
    return;
  }
  _attachedToClient.add(apClient);
  ...
  apClient.on('goal', async ({ playerName }) => {
    try {
      ...
      await dbExecute(
        'INSERT OR IGNORE INTO game_goals (gameId, playerName, completedAt) VALUES (?,?,?)',
