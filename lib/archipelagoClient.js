...
class ArchipelagoClient extends EventEmitter {
  constructor(port, knownPlayers = [], { ssl = false } = {}) {
    ...
    this._slotCandidates = [
      ...
      { name: 'Archipelago', game: 'Archipelago' },
      ...
      for (const packet of packets) {
        if (packet.cmd === 'RoomInfo') {
          ...
          if (packet.type === 'Goal') {
            ...
            const playerName = playerPart?.text
              ?? text.match(/^(.+?) \(Team #\d+\) has completed their goal/)?.[1]
              ?? null;
            const slot = playerPart?.player ?? this.players.find((p) => p.name === playerName)?.slot;
