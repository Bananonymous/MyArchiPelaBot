...
class ArchipelagoClient extends EventEmitter {
  constructor(port, knownPlayers = [], { ssl = false } = {}) {
    ...
    this._pendingHints = [];
    // Try the Archipelago spectator slot first, then fall back to real player slots
    this._slotCandidates = [
      { name: 'Archipelago', game: '' },
      ...knownPlayers.map((p) => ({ name: p.name, game: p.game ?? p.gameName ?? '' })),
    ];
    this._slotIndex = 0;
  }
    ...
    this.ws.on('message', (data) => {
      ...
      for (const packet of packets) {
        if (packet.cmd === 'RoomInfo') {
          const slot = this._slotCandidates[this._slotIndex] ?? { name: 'Archipelago', game: '' };
          ...
          // Fetch all current hints from server data storage (backfill missed/historical hints)
          try {
            const hintKeys = this.players
              .filter((p) => p.slot > 0)
              .map((p) => `_read_hints_${p.team ?? 0}_${p.slot}`);
            if (hintKeys.length > 0) {
              this.ws.send(JSON.stringify([{ cmd: 'Get', keys: hintKeys }]));
            }
          } catch (_) {}
          ...
          if (packet.type === 'Goal') {
            const playerPart = (packet.data || []).find((p) => p.type === 'player_id');
            if (playerPart) {
              const slot = parseInt(playerPart.text, 10);
              const player = this.players.find((p) => p.slot === slot);
              this.emit('goal', { text, playerName: player?.name ?? null, slot });
            }
          }
          if (packet.type === 'Hint') {
            const receiverSlot = packet.receiving;
