...
async function buildGlobalEmbed(gameId) {
  ...
  const lines = players.map((p) => {
    ...
    const apSlot = apClient?.players?.find((ap) => ap.name === p.name)?.slot;
    const totalLocs = apSlot != null ? apClient.getTotalLocationsForSlot(apSlot) : null;
    const checksStr = totalLocs != null ? `${sent}/${totalLocs}` : String(sent);
...
async function buildPersonalEmbed(gameId, player) {
  const [recvTotal, sentTotal, goalRow, recentItems, hints, outgoingHints] = await Promise.all([
    ...
    dbQueryAll(
      'SELECT senderName, itemName, locationName, flags FROM game_items WHERE gameId = ? AND receiverName = ? ORDER BY sentAt DESC LIMIT 20',
      [gameId, player.name]
    ),
    dbQueryAll(
  ...
  if (outgoingHints?.length) {
    outgoingList = outgoingHints.map((h) => {
      const icon = FLAG_ICON(h.flags);
  ...
  const apSlot2 = apClient2?.players?.find((ap) => ap.name === player.name)?.slot;
  const totalLocs2 = apSlot2 != null ? apClient2.getTotalLocationsForSlot(apSlot2) : null;
  const checksValue = totalLocs2 != null
    ? `${sentTotal?.count ?? 0}/${totalLocs2}`
    : String(sentTotal?.count ?? 0);
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: `Last ${Math.min(recentItems?.length ?? 0, 20)} items received`, value: itemList },
    )
  ...
  try {
    ...
    for (const player of players) {
      ...
      try {
        ...
        const threadMsg = await thread.send({ embeds: [await buildPersonalEmbed(gameId, player)] });
        await dbExecute(
  ...
  try {
    ...
    for (const tracker of trackers) {
      ...
      try {
        ...
        await msg.edit({ embeds: [await buildPersonalEmbed(gameId, player)] });
      } catch (e) {
...
module.exports = { setupTrackers, scheduleTrackerUpdate, buildGlobalEmbed, buildPersonalEmbed };
