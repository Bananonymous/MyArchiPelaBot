...
async function buildGlobalEmbed(gameId) {
  ...
  const lines = players.map((p) => {
    ...
    if (total == null && apClient) {
      ...
      if (apSlot) total = locationCounts[apSlot.name] ?? apClient.getTotalLocationsForSlot(apSlot.slot);
    }
...
async function buildPersonalEmbed(gameId, player) {
  ...
  if (total2 == null) {
    ...
    if (apClient2) {
      ...
      if (apSlot2) total2 = locationCounts2[apSlot2.name] ?? apClient2.getTotalLocationsForSlot(apSlot2.slot);
    }
  }
