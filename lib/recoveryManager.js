...
async function recoverRunningGames(discordClient) {
  ...
  for (const game of games) {
    ...
    // Refresh location counts if missing or previously parsed as empty
    let storedCounts = {};
    try { storedCounts = JSON.parse(game.locationCounts ?? '{}'); } catch {}
    if (Object.keys(storedCounts).length === 0) {
      const counts = await readLocationCounts(game.gameFile, players.map((p) => p.name));
